	...
	theme: localTheme({
		...
		plugins: [
			...
			containerPlugin({type: "tip", 
				before: (info: string, type): string => `<div class="custom-container tip"><i class="fas fa-info"></i>${
						info ? `<p class="custom-container-title">${info}</p>` : ""
					}\n`,
				after: (): string => "</div>\n",
			}),
			containerPlugin({type: "warning",
				before: (info: string, type): string => `<div class="custom-container warning"><i class="fas fa-exclamation"></i>${
						info ? `<p class="custom-container-title">${info}</p>` : ""
					}\n`,
				after: (): string => "</div>\n",
			}),
			containerPlugin({type: "danger",
				before: (info: string, type): string => `<div class="custom-container danger"><i class="fas fa-times"></i>${
						info ? `<p class="custom-container-title">${info}</p>` : ""
					}\n`,
				after: (): string => "</div>\n",
			}),
