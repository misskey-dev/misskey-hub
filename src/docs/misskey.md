...
## 分散型とは何か？
<b>分散(distributed)型</b>とは、<b>非中央集権(decentralized)</b>とも呼ばれ、コミュニティが多数のサーバー（Misskeyではインスタンスとも呼ばれます）に分散して存在し、それらが相互に<b>通信(連合、federation)</b>することでコンテンツ共有<b>ネットワーク(Fediverse)</b>を形成していることが特徴のサービスです。
単一のサーバーしか存在しない、もしくは複数存在しても互いに独立している場合は中央集権なサービスと言われ、例えばTwitterやFacebookなどほとんどのサービスがそれに該当します。
分散型のメリットは、自分に合った運営者やテーマのインスタンスを選択できることです。自分でインスタンスを作成することもできます。連合するおかげで、どのインスタンスを選んでも、同じコミュニティにアクセスできます。
...
また、インスタンスの運営者も、基本的には収益を得ていません。インスタンスの運営にはコストがかかるので、運営者の支援をすることもご検討ください。
開発には直接関係しませんが、インスタンスがあってこそのプロジェクトなので、運営が維持されるというのは開発と同じくらい重要なことです。
...
### 誰が運営していますか？
Misskeyは分散型なため、各インスタンスにそれぞれ異なった運営者がいます。従って、特定の個人や企業によって、Misskeyの全てが運営されているわけではありません。
また、開発チームが運営を行うわけでもないため、運営に関する連絡は、お使いのインスタンスの運営者に行ってください。
インスタンスの運営者は、インスタンスの情報ページで確認することができます。
あなたがインスタンスを作成すれば、あなたが運営者になります。

### どのインスタンスを選べばいいですか？
[インスタンス一覧が公開されています。](../instances.md)
インスタンスによってコミュニティのテーマ(特定のこと、ものが好き 等)が決められている場合があるので、自分に合ったテーマのインスタンスがあれば、そこを選ぶと良いかもしれません。
他にも、インスタンスの規模、ユーザー層、国および言語、運営者が信頼できるかどうか、などの観点があります。
なお、Misskey公式のインスタンスというものはありません。自身で新しくインスタンスを作成するという選択肢もあります。

基本的にどのインスタンスを選んだとしても、他の全てのインスタンスのユーザーと繋がることができます。

### インスタンスを建てるにはどうしたらいいですか？
Misskeyインスタンスの作成に興味を持っていただきありがとうございます。
2022年現在、Misskeyのホスティングサービスは存在しないため、インスタンスの作成にはある程度の知識が必要です。
インスタンスの作成方法については[こちら](./install.md)をご覧ください。
...
詳しくは[こちら](../appendix/assets.md)をご覧ください。

### 時折目にする猫耳の可愛い女の子は？
Misskeyの守り神、藍ちゃんです。ｱｲﾁｬﾝｶﾜｲｲﾔｯﾀｰ!

::: tip
藍ちゃんについては[こちら](https://xn--931a.moe/)をご覧下さい。
:::
